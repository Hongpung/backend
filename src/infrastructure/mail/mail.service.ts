...
export class MailService {
  ...
  private readonly isDevelopEnv = process.env.NODE_ENV === 'develop';
  private transporter?: nodemailer.Transporter;

  constructor() {
    if (this.isDevelopEnv) {
      this.logger.log('develop 환경에서 메일 전송을 mock 처리합니다.');
      return;
    }
  ...
  private async sendMail(
    ...
    if (this.isDevelopEnv) {
      this.logger.debug(
        `[MAIL MOCK][${context}] to=${String(mailOptions.to)} subject=${String(mailOptions.subject)}`,
      );
      return {
        response: 'MOCKED_IN_DEVELOP',
  ...
  async sendEmailConfirmMail(email: string, verificationCode: number) {
    ...
    try {
      const info = await this.sendMail(mailOptions, 'sendEmailConfirmMail');
      return info;
    } catch (error) {
  ...
  async sendPasswordModifyMail(email: string, verificationCode: number) {
    ...
    try {
      const info = await this.sendMail(mailOptions, 'sendPasswordModifyMail');
      return info;
    } catch (error) {
      this.logger.error('Error sending email:', error);
      throw error;
  ...
  async sendSignUpAcceptedMail(email: string, name: string) {
    ...
    try {
      const info = await this.sendMail(mailOptions, 'sendSignUpAcceptedMail');
      return info;
    } catch (error) {
      this.logger.error('Error sending email:', error);
      throw error;
  ...
  async sendSignUpRequestedMail(email: string, requestCount: number) {
    ...
    try {
      const info = await this.sendMail(mailOptions, 'sendSignUpRequestedMail');
      return info;
    } catch (error) {
      this.logger.error('Error sending email:', error);
      throw error;
